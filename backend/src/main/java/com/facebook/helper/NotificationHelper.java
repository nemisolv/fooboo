...
public class NotificationHelper {
    ...
    private void createAndSendNotification(NotificationType type, User actor, User recipient, String identifier) {
        Notification newNotification = Notification.builder()
                .actor(actor)
                .recipient(recipient)
                .content(Utility.generateContentLink(type, actor.getFirstName() + " " + actor.getLastName()))
    ...
    public void buildPostNotification(NotificationType type, Post post, User actor, String identifier) {
        switch (type) {
            case NEW_POST -> {
                if (post.getScope() == PostScope.FRIENDS) {
                    post.getAuthor().getFriends().forEach(friend ->
                            createAndSendNotification(type, actor, friend, identifier)
                    );
                }
            }
            case SHARED_POST -> {
    ...
    public void buildFriendNotification(NotificationType type, User sender, User recipient, String identifier) {
        createAndSendNotification(type, sender, recipient, identifier);
    }
}
